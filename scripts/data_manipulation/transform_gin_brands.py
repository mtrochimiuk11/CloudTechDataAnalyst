#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Transformacja nazw marek ginu do postaci keywordow dla estymatora.

Wejscie:  data/gin_brands.csv                       (kolumny `brand`, `extraInfo`)
Wyjscie:  data/transformed/gin_brands_transformed.csv  (kolumny `brand`, `extraInfo`, `keyword`)

Kolumna `extraInfo` jest przepuszczana 1:1 (passthrough) -- to opis (kraj, styl,
botanikale itd.), pozniej zrodlo slow `optional` przy budowie zapytan.
...
KONFIGURACJA. Skan znakow potwierdzil: apostrofy ("Blackwood's" -> "blackwoods",
"Gordon's" -> "gordons"), kropki ("Archie Rose Distilling Co." -> "archie-rose-
distilling-co"), '&' tworzacy dwa warianty ("Lind & Lime" -> "lind-lime" ORAZ
"lind-and-lime") oraz diakrytyk 'ä' ("Steinhäger" -> "steinhager"). Generyczne
slowo "Gin" jest ZACHOWANE: "Boodles British Gin" -> "boodles-british-gin".

Uruchomienie (z dowolnego cwd -- sciezki liczone wzgledem __file__):
    python3 scripts/data_manipulation/transform_gin_brands.py
"""
...
PLIK_WEJSCIOWY = DATA_DIR / "gin_brands.csv"
KATALOG_WYJSCIOWY = DATA_DIR / "transformed"
PLIK_WYJSCIOWY = KATALOG_WYJSCIOWY / "gin_brands_transformed.csv"
...
def utnij_dopisek(nazwa: str) -> str:
    ...
    " home of "). Niespacjowany myslnik (np. "Lind-Lime") ZOSTAJE.
    """
    pozycje = []
