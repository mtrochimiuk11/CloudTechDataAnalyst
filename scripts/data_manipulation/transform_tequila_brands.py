#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Transformacja nazw marek tequili do postaci keywordow dla estymatora.

Wejscie:  data/tequila_brands.csv                       (kolumny `brand`, `extraInfo`)
Wyjscie:  data/transformed/tequila_brands_transformed.csv  (kolumny `brand`, `extraInfo`, `keyword`)

Kolumna `extraInfo` jest przepuszczana 1:1 (passthrough) -- to glownie rok zalozenia
(czasem pusty, np. "1870, defunct"); zachowana dla kompletu.
...
KONFIGURACJA. Skan znakow potwierdzil: same diakrytyki (ó, é, ñ, í) -- wszystkie
rozkladaja sie przez NFKD ("Patrón" -> "patron", "DeLeón" -> "deleon", "Señor Río"
-> "senor-rio"). Cyfry w nazwie sa ZACHOWANE: "1800 Tequila" -> "1800-tequila",
"4 Copas" -> "4-copas", "Tequila 125" -> "tequila-125". Generyczne slowo
"Tequila"/"tequila" jest ZACHOWANE.

Uruchomienie (z dowolnego cwd -- sciezki liczone wzgledem __file__):
    python3 scripts/data_manipulation/transform_tequila_brands.py
"""
...
PLIK_WEJSCIOWY = DATA_DIR / "tequila_brands.csv"
KATALOG_WYJSCIOWY = DATA_DIR / "transformed"
PLIK_WYJSCIOWY = KATALOG_WYJSCIOWY / "tequila_brands_transformed.csv"
# Kolumna z nazwa marki oraz kolumny przepuszczane 1:1 (passthrough).
KOLUMNA_NAZWY = "brand"
KOLUMNY_PASSTHROUGH = ["extraInfo"]
...
def utnij_dopisek(nazwa: str) -> str:
    ...
    " home of "). Niespacjowany myslnik ZOSTAJE. Uwaga: cyfry w nazwie sa
    czescia marki -- ABV "<liczba>%" wymaga znaku '%', wiec "1800 Tequila" nie
    jest obcinane.
    """
    pozycje = []
